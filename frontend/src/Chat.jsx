...
import { Send, Menu, X, Search, MoreVertical, Phone, Video, Paperclip, Smile, Camera, ShieldCheck, Bell, UserPlus, Check } from 'lucide-react';
...
const Chat = ({ onLogout }) => {
  ...
  // --- डेटाबेस से प्रोफाइल और दोस्तों की लिस्ट लाना ---
  const fetchProfile = async () => {
    ...
    try {
      ...
      if(res.data.name && res.data.name !== 'Agent') {
        ...
        localStorage.setItem('candy_userName', res.data.name);
      }
      if(res.data.avatar) {
        setMyProfilePic(res.data.avatar);
        localStorage.setItem('candy_userPic', res.data.avatar);
      }
  ...
  // --- डेटाबेस में प्रोफाइल (Name/DP) सेव करना ---
  const saveProfileToDB = async (updatedName, updatedPic) => {
    if (!myEmail) return;
    try {
      await axios.put(`${BACKEND_URL}/api/users/update`, {
        email: myEmail,
        name: updatedName,
        avatar: updatedPic
      });
  ...
  useEffect(() => {
    socket.on('receive_message', (data) => {
      setChats((prevChats) => {
        if (prevChats.some(msg => msg.id === data.id)) return prevChats;
  ...
  const handleSend = async (e) => {
    e.preventDefault();
    if (!message.trim() || !selectedChat || !currentRoomId) return;
    ...
    <div className="flex h-[100dvh] bg-[#050505] text-slate-200 overflow-hidden font-sans relative">
      ...
      {!selectedChat && (
        <div className="absolute top-4 left-4 md:hidden z-30">
          <button onClick={() => setIsSidebarOpen(true)} className="p-3 text-slate-300 hover:text-white bg-slate-900/80 rounded-xl backdrop-blur-md border border-slate-700 shadow-lg">
            <Menu size={24} />
          </button>
        </div>
      )}
      ...
      <AnimatePresence>
        {(isSidebarOpen || window.innerWidth >= 768) && (
          <motion.div
            ...
            className={`fixed inset-y-0 left-0 z-50 w-72 lg:w-80 bg-slate-950/80 backdrop-blur-2xl border-r border-slate-800/50 flex flex-col transform md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}
          >
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar px-3 space-y-1 mt-2">
              {allContacts.map((user) => (
                <motion.div 
                  ...
                  onClick={() => { setSelectedChat(user); setIsSidebarOpen(false); }}
                  className={`flex items-center p-3 rounded-xl cursor-pointer transition-colors border ${selectedChat?._id === user._id ? 'bg-slate-800 border-slate-700/50 shadow-lg' : 'border-transparent hover:border-slate-700/50'}`}
                >
                  <div className="relative flex-shrink-0">
                    ...
                    {user.online && <span className="absolute bottom-0 right-0 w-3 h-3 bg-emerald-500 border-2 border-slate-950 rounded-full shadow-[0_0_10px_rgba(16,185,129,0.8)]"></span>}
                  </div>
            ...
            <div className="p-4 border-t border-slate-800/50 bg-slate-900/30 flex flex-col gap-3">
               <div className="flex items-center gap-3 bg-slate-900 p-3 rounded-xl border border-slate-800">
                  ...
                  <div className="flex-1 overflow-hidden">
                    <input 
                      ...
                      onChange={(e) => { setMyName(e.target.value); localStorage.setItem('candy_userName', e.target.value); }} 
                      onBlur={() => saveProfileToDB(myName, myProfilePic)} // बाहर क्लिक करते ही सेव
                      className="text-sm font-bold text-white bg-transparent border-b border-transparent hover:border-slate-600 focus:border-blue-500 focus:outline-none w-full pb-0.5 transition-colors truncate" 
               ...
               <button onClick={onLogout} className="w-full py-2.5 bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 rounded-xl text-sm font-medium transition-all">
                  Logout
               </button>
            </div>
          </motion.div>
          ...
          <>
            <header className="flex-shrink-0 h-16 md:h-20 px-4 md:px-8 border-b border-slate-800/50 bg-slate-950/40 backdrop-blur-xl flex items-center justify-between z-20">
              <div className="flex items-center gap-3 md:gap-4">
                <button className="md:hidden p-1.5 -ml-2 rounded-lg text-slate-400 hover:bg-slate-800/50 transition-colors" onClick={() => setIsSidebarOpen(true)}>
                  <Menu size={24} />
                </button>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 md:p-8 space-y-5 custom-scrollbar">
              <AnimatePresence>
                {currentChats.map((chat, index) => (
                  ...
                  >
                    {chat.sender === 'other' && (
                       <span className="text-[10px] md:text-[11px] text-slate-400 mb-1 ml-9 md:ml-12 font-medium tracking-wide">
                         {chat.senderName || selectedChat.name}
                       </span>
                    )}
                    ...
                    <div className="flex items-end gap-2 max-w-[85%] md:max-w-[70%]">
                      {chat.sender === 'other' && (
                        <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-slate-800 flex-shrink-0 border border-slate-700 overflow-hidden shadow-lg">
                           ...
                           : selectedChat.avatar ? ( <img src={selectedChat.avatar} className="w-full h-full object-cover" /> ) 
                           : ( <div className="w-full h-full flex items-center justify-center text-xs font-bold text-slate-300">{(chat.senderName || selectedChat.name).charAt(0).toUpperCase()}</div> )}
                        </div>
                      )}
                      
                      <div className={`p-3 md:p-4 rounded-2xl ${
                        chat.sender === 'me' 
                        ? 'bg-gradient-to-br from-blue-600 to-purple-600 text-white rounded-br-sm shadow-[0_5px_20px_rgba(124,58,237,0.2)]' 
                        : 'bg-slate-900/80 border border-slate-800/50 text-slate-200 rounded-bl-sm backdrop-blur-md'
                      }`}>
                        <p className="text-[14px] md:text-[15px] leading-relaxed break-words">{chat.text}</p>
                      </div>
                      ...
                      {chat.sender === 'me' && (
                        <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-slate-800 flex-shrink-0 border border-slate-700 overflow-hidden shadow-lg shadow-purple-500/20">
                          {myProfilePic ? ( <img src={myProfilePic} className="w-full h-full object-cover" alt="Me" /> ) 
                          : ( <div className="w-full h-full flex items-center justify-center text-xs font-bold text-white">{myName.charAt(0).toUpperCase()}</div> )}
                        </div>
                      )}
                    </div>
            ...
            <div className="flex-shrink-0 p-3 md:p-4 bg-slate-950/80 backdrop-blur-xl border-t border-slate-800/50">
              <form onSubmit={handleSend} className="max-w-4xl mx-auto flex items-end gap-2 bg-slate-900/80 border border-slate-700/50 p-1.5 md:p-2 rounded-3xl shadow-xl focus-within:border-purple-500/50 transition-colors">
                <textarea
                  value={message} onChange={(e) => setMessage(e.target.value)}
                  placeholder="Type a secure message..."
                  className="flex-1 bg-transparent border-none text-slate-200 placeholder-slate-500 focus:outline-none resize-none py-2.5 md:py-3.5 px-3 md:px-4 max-h-24 min-h-[44px] text-[14px] md:text-[15px]"
                  rows="1" onKeyDown={(e) => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(e); } }}
                />
                <motion.button 
                  whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  type="submit" disabled={!message.trim()}
                  className="p-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-2xl shadow-lg shadow-purple-500/25 disabled:opacity-50 disabled:cursor-not-allowed m-1 transition-all"
                >
